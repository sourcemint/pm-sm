...
SM.prototype.require = function(locator, callback) {
	...
	try {
		...
		if (path && PATH.existsSync(path)) {
			// TODO: If found, load descriptor to resolve `libDir` and call (skip `sm.install()` below).
//			locator.location = path;
    ...
    PM.forProgramPath(self.packagePath).then(function(pm) {
    	...
    	if (typeof locator.module !== "undefined") {
			callback(null, require(PATH.join(manifest[packageURI].status.path, manifest[packageURI].summary.libDir, locator.module)));
    	}
    	else {
    		// TODO: Use `pinf` to load descriptor.
    		var descriptor = JSON.parse(FS.readFileSync(PATH.join(manifest[packageURI].status.path, "package.json")));

    		if (descriptor.main) {
				callback(null, require(PATH.join(manifest[packageURI].status.path, descriptor.main)));
    		}
    		else {
				callback(null, null);
    		}
