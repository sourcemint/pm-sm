...
exports.main = function(pm, options) {
    ...
    return git.status().then(function(status) {
        ...
        if (status.type === "git") {
            ...
            if (status.tagged !== "v" + pm.context.program.package.descriptor.json.version) {
                TERM.stderr.writenl("\0red(\0bold(ERROR: Cannot publish as latest GIT tag does not match 'v" + pm.context.program.package.descriptor.json.version + "' based on version from package descriptor '" + pm.context.program.package.descriptor.path + "'!\0)\0)");
                return;
            }
