...
exports.main = function(pm, options) {
    ...
    if (visited[pm.context.package.path]) {
        return done;
    }
    visited[pm.context.package.path] = true;
                ...
                return pm.context.program.walkPackages(options, function(parentPkg, pkgInfo, context) {
                    return SM_PM.forPackagePath(pkgInfo[0].path, pm).then(function(pm) {
                        ...
                        return pm.status(opts).then(function(status) {
                            if (status.type === "git") {
                                if (status.dirty === false && !status.tagged) {
...
console.log("skip bump for: " + pm.context.package.path);        
return;        
        ...
        return git.status().then(function(status) {
            ...
            return pm.context.package.descriptor.write().then(function() {
                ...
                if (pm.context.package.descriptor.json.shrinkwrap === true) {
                    done = Q.when(done, function() {
                        TERM.stdout.writenl("\0cyan(Shrinkwrapping package: '" + pm.context.package.path + "\0)");
                        ...
                        EXEC("npm shrinkwrap", {
                            ...
                            if (UTIL.trim(stdout) !== "wrote npm-shrinkwrap.json") {
                                deferred.reject(new Error("writing shrinkwrap file: " + stdout + stderr));
                                return;
                            }
                            deferred.resolve();
                        });
