...
function checkForUpdate(pm, options) {
    ...
    var cache = new URL_PROXY_CACHE.UrlProxyCache(PATH.join(pm.context.homeBasePath, "url-cache"), {
        ...
        ttl: (new Date(timeNow.getFullYear(), timeNow.getMonth(), timeNow.getDate()).getTime())*-1    // Today
    });
    return cache.get("https://registry.npmjs.org/sm").then(function(response) {
        ...
        EXEC("sm --version", function(err, stdout, stderr) {
            ...
            if (SEMVER.compare(actualVersion, latestVersion) < 0) {
                ...
                TERM.stderr.writenl(" !  " + "A NEW VERSION (" + latestVersion + ") of `sm` is available! You are running '" + actualVersion + "'. Please update:");
                TERM.stderr.writenl(" !");
                TERM.stderr.writenl(" !  \0magenta(" + "    npm update -g sm@>=" + latestVersion + "\0)");
                TERM.stderr.writenl(" !");
                TERM.stderr.writenl(" !  " + "If you run into problems after the update you can downgrade again:");
                TERM.stderr.writenl(" !");
                TERM.stderr.writenl(" !  \0magenta(" + "    npm install -g sm@" + actualVersion + "\0)");
