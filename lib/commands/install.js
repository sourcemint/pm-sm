...
exports.main = function(pm, options) {
    ...
    return PACKAGES.loadDependenciesForProgram(pm.context.program).then(function() {
        ...
        return pm.context.program.walkPackages({}, function(parentPkg, pkgInfo, pkgContext) {
            ...
            if (pkgContext.level === 0) {
                // We are in the top package.

                // TODO: Git pull if git not dirty or ahead.
                return;
            } else
            if (pkgContext.level > 1) {
                // We are in a transitive package which we ignore as we call `sm update` on each package at level 1.
                return;
            }
            // Not needed as we only process level 1 package but here just in case we need it later.
            //if (pkgContext.circular === true) return;

            if (options.verbose) {
                TERM.stdout.writenl("\0cyan(Updating package at: " + pkgInfo[0].path + "\0)");
            }

            // We only process level 1 packages.
            ...
            function walkMappings() {
                ...
                if (mapping === ".") {
                    ...
                    return PM.forPackagePath(pkgInfo[0].path, pm).then(function(pm) {
                        ...
                        if (PATH.existsSync(args.locator)) {
                            // The locator is an absolute path.
                            manifest[alias] = {
                                path: args.locator,
                                // TODO: This libDir must be read from descriptor.
                                libDir: libDir
                            };
                            return Q.ref();
                        }
                        ...
                        return pm.path(args).then(function(path) {
                            ...
                            return Q.when(deferred.promise, function() {
                                ...
                                if (options.update === true) {
                                    ...
                                    TERM.stdout.writenl("\0cyan(Running: sm " + args.join(" ") + " (cwd: " + path + ")\0)");
...
//console.log("UPDATE", pkgInfo[2][0], dependency, pkgInfo[0].path);
            }
            ...
            if (pmDeclaration !== false && (typeof pmDeclaration === "undefined" || pmDeclaration === "npm") && options["no-native-install"] !== true) {
                return Q.call(function() {
                    ...
                    if (options.update === true) {
// DEPRECATED: 'latest' check is old. `update` always checks for latest packages now.
//                        if (options.latest === true) {
                            // NOTE: We do an `install` first and then an update so that `devDependencies` get installed.
                            // TODO: Only run `install` for root package.
