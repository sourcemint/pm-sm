...
*/
    ...
    return CORE.getStatusTree(pm, options).then(function(statusTree) {
        ...
        function writeMessage(message) {
            if (previousMessages[message]) return;
            previousMessages[message] = true;
        ...
        return statusTree.forEachNode(function(node) {
            ...
            if (node.status.inheritedStatus.symlinked) {
                writeMessage("\0yellow([sm] SKIP: Not updating '" + node.path + "' (parent linked in from outside program root)\0)");
                return false;   // Don't traverse deeper.
            }
            if (node.status.status.symlinked) {
                writeMessage("\0yellow([sm] SKIP: Not updating '" + node.path + "' (linked in from outside program root)\0)");
                return false;   // Don't traverse deeper.
            }

            if (node.status.inheritedStatus.vcs) {
                writeMessage("\0yellow([sm] SKIP: Not updating '" + node.path + "' (parent under version control)\0)");
                return false;   // Don't traverse deeper.
            }
            if (node.status.status.vcs) {
                writeMessage("\0yellow([sm] SKIP: Not updating '" + node.path + "' (under version control)\0)");
                return false;   // Don't traverse deeper.
            }
