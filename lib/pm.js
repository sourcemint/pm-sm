...
var PM = function() {
    ...
    this.context.credentials = new CREDENTIALS(PATH.join(this.context.homeBasePath, "credentials.json"), "default");
}
...
//            self.context.deployment = DESRIPTOR from self.context.program.descriptor.path.replace(/(\.json$)/, "." + target + "$1");
            ...
            if (owningPM) {
                ...
                if (self.context.program.descriptor.json.config &&
                    ...
                    self.context.credentials = new CREDENTIALS(path, self.context.program.descriptor.json.config["github.com/sourcemint/profile/0"].name);
                }
            }
        });
    });
