...
exports.getStatusTree = function(pm, options) {
    ...
    Node.prototype.update = function() {
    	...
    	if (!self.status.status.installed) {
            TERM.stdout.writenl("\0cyan([sm] Installing missing package '" + self.path + "' from '" + self.status.locator.location + "'\0)");
    	} else
        if (self.status.status.newInVersion) {
            TERM.stdout.writenl("\0cyan([sm] Updating outdated package '" + self.path + "' to '" + self.status.summary.newInSelectorLocation + "'\0)");
        }
                                        ...
                                        FS.lstat(path, function(err, stats) {
                                            ...
                                            if (stats.isSymbolicLink()) {
                                            	...
                                            	if (!/^\//.test(linkStr)) {
                                            		...
                                            		if (PATH.join(basePath, linkStr).substring(0, basePath.length) === basePath) {
