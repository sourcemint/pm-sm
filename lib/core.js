...
exports.getStatusTree = function(pm, options) {
    ...
    Node.prototype.assemble = function(context, inheritedStatus) {
        ...
        var info = {
            ...
            vcs: false,
            relpath: self.path.substring(pm.context.program.package.path.length + 1)
        };
    ...
    Node.prototype.update = function() {
    	...
    	if (!self.status.status.installed) {
            TERM.stdout.writenl("\0cyan([sm] Installing missing '" + self.status.status.relpath + "' from '" + self.status.locator.location + "'\0)");
    	} else
