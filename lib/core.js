    ...
    Node.prototype.initFromFsNode = function(node) {
        ...
        if (self.status.locator && self.status.locator.viaPm === "npm" && self.status.descriptor) {
            if (
                ...
                if (SEMVER_NPM.valid(pointer) && self.status.npm.descriptor.versions[pointer]) {
    ...
    Node.prototype.edit = function() {
        ...
        return SM_PM.forPackagePath(self.path, pm).then(function(pm) {
            ...
            return Q.when(pm.edit(opts), function() {
                // We don't refresh for now as we exit after the edit command.
                // return self.refresh(options);
            });
        });
    }
    ...
    var Node = function(name, level) {
        ...
        self.addLoader("git", false, function(options) {
            return SM_PM.forPackagePath(self.path, pm).then(function(pm) {
                ...
                // If locator has a selector version and we are not a git repo
                // we check if we ahve a git locator and lookup latest version in cache.
                ...
                if (gitInfo.fromCache) {
                    ...
                    // TODO: Set `self.status.locator.version` as above. We don't do that right now
                    //       because if this package is a git repo we don't update or manipulate it anyway
                    //       and the current git ref is king. We may want to set this to display
                    //       latest resolved version on status for info purposes.
                }
