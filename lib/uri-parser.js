...
vendors["github.com"] = function(parsedUri) {
    ...
    if (parsedUri.protocol === "git:") {
        ...
        if (parsedUri.hash) {
            if (/^#\//.test(parsedUri.hash)) {
                throw new Error("Not a valid github.com URL! Hash/branch '" + parsedUri.hash.substring(1) + "' may not begin with '/'!");
            }
    ...
    if((m = parsedUri.path.match(/^\/([^\/]*)\/([^\/]*)\/?((tarball|zipball|tree|commit)\/(.*?))?\/?$/))) {
